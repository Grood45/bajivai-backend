...
//   try {
//     const { answer, user_ids } = req.body;
//     const userIdObjects = user_ids.map((userId) => new mongoose.Types.ObjectId(userId));
...
//     for (const bet of betsToUpdate) {
//       const user = await User.findOne({ user_id: bet.user_id }).session(session);
